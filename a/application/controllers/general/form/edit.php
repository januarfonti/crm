...
class edit extends AD_Controller {
	function __construct()
	{
		...
		$this->field_query	= $this->db->query('
			...
			ON a.type = b.id
			WHERE a.form = ' . $this->fid . ' ORDER BY orders ASC ' );
	...
	{	
		...
		if ($this->form_validation->run() == FALSE)  
			{ $this->_render('general/form/edit'); }
		else 
		{			
			$form_data = array();	
			foreach ($this->field_result as $this->f)
			{ $form_data[$this->f->field] = set_value($this->f->field); } 			
						
			$this->load->model('form/update');			
			if ($this->update->saveform($form_data) == TRUE) { redirect($this->base . $this->form_result->success.'/#updated');  }
